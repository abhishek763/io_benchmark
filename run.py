...
inp_files = [ '/home/ubuntu/read.bin']
    ...
    for inp_file in inp_files:
        for buf_size in buf_sizes:
            for num_concurrent in num_concurrents:
                ...
                cmd = "./target/release/async-io -i {0} -b {1} -n {2} -r -t {3}".format(inp_file, buf_size, num_concurrent, time)
...
with open('read-async-random.csv', 'w') as f:
    ...
    for inp_file in inp_files:
        for buf_size in buf_sizes:
            for num_concurrent in num_concurrents:
                subprocess.Popen(clearCacheCmd.split(), shell=True)
                cmd = "./target/release/async-io -i {0} -b {1} -n {2} -r --random -t {3}".format(inp_file, buf_size, num_concurrent, time)
                e, out = run_benchmark(cmd)
                if e == 0:
                    f.write('{0},{1},{2},{3}\n'.format(inp_file, buf_size, num_concurrent, out))
